...
//        accountService.updateBalance(externalTransaction.getSenderAccount(), netAmount);
    ...
    public TransactionInitDTO createInternal(CreateInternalDTO create) {
        ...
        try {
            ...
            receiverAccount = accountDubboService.getAccountDetail(create.getReceiverAccount());

            if (receiverAccount == null || !receiverAccount.getStatusAccount().equals(ObjectStatus.ACTIVE))
                throw new Exception();

            log.info("Receiver account information with account number({}): {}", create.getReceiverAccount(), receiverAccount);
        } catch (Exception e) {
...
//        accountService.updateBalance(externalTransaction.getSenderAccount(), netAmount.negate());
    ...
    private void rollbackInternal(InternalTransaction internalTransaction) {
        ...
        try {
            ...
            receiverAccount = accountDubboService.getAccountDetail(internalTransaction.getReceiverAccount());

            if (receiverAccount == null || !receiverAccount.getStatusAccount().equals(ObjectStatus.ACTIVE))
                throw new Exception();

            log.info("Receiver account information with account number({}): {}", internalTransaction.getReceiverAccount(), receiverAccount);
        } catch (Exception e) {
...
//        accountService.updateBalance(internalTransaction.getReceiverAccount(), receiverNetAmount.negate());
    ...
    private void rollbackPayment(PaymentTransaction paymentTransaction) {
        ...
        try {
            ...
            receiverAccount = accountDubboService.getAccountDetail(paymentTransaction.getReceiverAccount());

            if (receiverAccount == null || !receiverAccount.getStatusAccount().equals(ObjectStatus.ACTIVE))
                throw new Exception();

            log.info("Receiver account information with account number({}): {}", paymentTransaction.getReceiverAccount(), receiverAccount);
        } catch (Exception e) {
